...
/*   Updated: 2019/10/17 19:23:32 by lnicosia         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
Form::Form(std::string name, std::string target, int minSignGrade, int minExecGrade):
    _name(name), _target(target), _signed(false), _minSignGrade(minSignGrade), _minExecGrade(minExecGrade)
{
    if (minSignGrade < 1)
...
void                Form::executeCheck(Bureaucrat const & b) const
{
    if (this->getSigned() == false)
        throw FormNotSignedException();
    if (b.getGrade() > this->getMinExecGrade())
        throw GradeTooLowException("Bureaucrat grade is too low to execute the form");
}
...
Form::FormNotSignedException::FormNotSignedException(FormNotSignedException const & instance)
{
    *this = instance;
}
